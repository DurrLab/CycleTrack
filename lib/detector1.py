...
#from utils.tracker_sort import Tracker
from utils.sort_track import Tracker
from utils.sort import Sort
from dataset.dataset_factory import get_dataset


class Detector(object):
  def __init__(self, opt,rematch=True):
    if opt.gpus[0] >= 0:
  ...
  def run(self, image_or_path_or_tensor, meta={}):
    ...
    for scale in self.opt.test_scales:
      ...
      if not pre_processed:
        ...
        if 'mask' in pre_processed_images:
            mask=pre_processed_images['mask'].numpy()
            mask=mask[0,:,:,:]
    ...
    if self.opt.tracking:
      ...
      if self.opt.tracking_base:     
        trackings = np.array([det['tracking'] for det in results if hasattr(det['tracking'], "__len__")], np.float32)
        if len(self.tracking_base)==0:
          track_mean=np.mean(trackings,axis=0)
          self.tracking_base = np.mean(np.array([track for track in trackings if np.sqrt(sum(track**2))>0.1*np.sqrt(sum(track_mean**2))]),axis=0)
        else:
          trackings = np.array([track for track in trackings if np.dot(track,self.tracking_base)>0],np.float32)
          if len(trackings)>0:
            track_mean=np.mean(np.array([track for track in trackings if np.sqrt(sum(track**2))>0.1*np.sqrt(sum(self.tracking_base**2))]),axis=0)
          else:
            track_mean=self.tracking_base
          for i,det in enumerate(results):
            if not hasattr(det['tracking'], "__len__"):
              results[i]['tracking']=self.tracking_base
              continue
            upd_ratio=np.dot(det['tracking'],self.tracking_base)/np.sqrt(sum(det['tracking']**2)*sum(self.tracking_base**2))
            if upd_ratio < 0:
              upd_ratio=0
            if upd_ratio > 1.:
              upd_ratio=1.
            
            if  math.isnan(upd_ratio):
              results[i]['tracking']=self.tracking_base
            
            if not math.isnan(upd_ratio) and upd_ratio < 0.2:
              results[i]['tracking']=upd_ratio * det['tracking'] + (1-abs(upd_ratio)) * self.tracking_base     
                
          trackings = np.array([det['tracking'] for det in results if hasattr(det['tracking'], "__len__") and np.sqrt(sum(det['tracking']**2))>0.5*np.sqrt(sum(self.tracking_base**2))], np.float32)
          if len(trackings) > 0:
            self.tracking_base = np.mean(trackings,axis=0)
          print('********************')
          print(self.tracking_base)
          print('********************')
      
      if len(results)>0:
        self.velocity=np.mean(np.array([np.sqrt(det['tracking'][0]**2+det['tracking'][1]**2) for det in results 
                                        if hasattr(det['tracking'], "__len__")], np.float32))
      results, results_sort = self.tracker.step(results, public_det)
      self.pre_images = images
      CT_result=np.array([np.concatenate([result['bbox'],[result['tracking_id']]]) for result in results])
      '''
    ...
    if self.opt.debug >= 1:
        if 'mask' in pre_processed_images:
            self.show_results(self.debugger, mask, results)
        else:
    ...
    ret = {'results': results, 'velocity':self.velocity,'tot': tot_time, 'load': load_time,
