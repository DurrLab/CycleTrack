...
class Tracker(object):
  ...
  def step(self, results, public_det=None):
    ...
    invalid = ((dist > 8.0 * self.opt.track_size * track_size.reshape(1, M)) +\
      (dist > 8.0 * self.opt.item_size * item_size.reshape(N, 1)) + \
      (item_cat.reshape(N, 1) != track_cat.reshape(1, M))) > 0
    if M > 1:
        cells_dist=(((tracks.reshape(1, -1, 2) - \
                  tracks.reshape(-1, 1, 2)) ** 2).sum(axis=2)) # M x M
        cells_dist=np.median(cells_dist,axis=1)
        invalid += (dist > 8.0 * self.opt.cells_dist * cells_dist.reshape(1, M))
    ...
    if self.rematch:
        for old in self.tracks:
          if old['active']==1 and not (old['tracking_id'] in matched_ids) and old['frame']>1:
      ...
      for i in unmatched_dets:
        ...
        if track['score'] > self.opt.new_thresh:
          if self.rematch and len(self.rematch_ids)>0:
            ...
            self.rematch_ids=self.rematch_ids[1:]
          else:
            self.id_count += 1
            track['tracking_id'] = self.id_count
          track['age'] = 1
