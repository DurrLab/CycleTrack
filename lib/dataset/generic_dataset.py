...
class GenericDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    opt = self.opt
    img, anns, img_info, img_path, _ = self._load_data(index)
  ...
  def _load_image_anns(self, img_id, coco, img_dir):
    ...
    img=np.array(img)
    return img, anns, img_info, img_path,raw

  def _load_data(self, index):
    coco = self.coco
    img_dir = self.img_dir
    img_id = self.images[index]
    img, anns, img_info, img_path, raw = self._load_image_anns(img_id, coco, img_dir)
    return img, anns, img_info, img_path, raw
  ...
  def _load_pre_data(self, video_id, frame_id, sensor_id=1):
    ...
    if 'train' in self.split:
      img_ids = [(img_info['id'], img_info['frame_id']) \
          ...
          img_info['frame_id'] != frame_id and \
  ...
  def _get_aug_param(self, c, s, width, height, disturb=False):
    if (not self.opt.not_rand_crop) and not disturb:
      ...
      w_border = self._get_border(600, width)
      h_border = self._get_border(300, height)
